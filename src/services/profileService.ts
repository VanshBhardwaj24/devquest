...
export const profileService = {
  ...
  async updateProfile(userId: string, updates: Partial<User>) {
    ...
    try {
      ...
      // Try to find a single profile row for this user first (limit to 1)
      ...
      if (selectError) {
        console.error('Error finding existing profile for update:', selectError);
        throw selectError;
      }

      // If no existing profile, create one (use updateData plus user_id)
      if (!existing) {
        ...
        if (insertError) {
          console.error('Error inserting new profile during update fallback:', insertError);
          throw insertError;
        }
      ...
      // Update the single profile row by its primary key `id` to ensure only one row is modified
      const { data, error } = await supabase
        .from('profiles')
        .update(updateData)
        .eq('id', existing.id)
