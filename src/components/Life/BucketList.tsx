    ...
    <div className={`p-4 md:p-8 ${darkMode ? 'text-white' : 'text-gray-900'} min-h-screen`}>
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <Button className="bg-orange-500 text-black font-bold font-mono border-2 border-black brutal-shadow">
            ...
            <div key={item.id} className={`relative group border-4 ${item.completed ? 'border-lime-500' : darkMode ? 'border-gray-800' : 'border-black'} ${darkMode ? 'bg-gray-900' : 'bg-white'} brutal-shadow transition-all hover:-translate-y-1`}>
              ...
              <div className="p-4">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className={`w-6 h-6 border-2 border-black flex items-center justify-center ${item.completed ? 'bg-lime-500' : 'bg-transparent'}`}>
                ...
                <div className="flex justify-between items-center mt-4">
                   ...
                   <Button size="sm" variant="ghost" className="h-6 text-xs font-mono uppercase hover:bg-orange-500 hover:text-black">
                     Edit
                   </Button>
                </div>
              </div>
