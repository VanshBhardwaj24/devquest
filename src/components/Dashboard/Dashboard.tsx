...
import { 
  ...
  Cpu, Battery, Signal, Wifi, Menu
} from 'lucide-react';
// NonNegotiables removed from dashboard view per request
...
export function Dashboard() {
  const { state } = useApp();
  const { user, xpSystem, tasks, codingStats, darkMode, systemLogs } = state;
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <span className="font-black text-xs flex items-center gap-2 uppercase">
                  <Activity className="h-4 w-4" /> HP Status
                </span>
                <span className="font-black text-xs bg-red-500 text-white px-1 border border-black">
                  {user?.hp || 50}/{user?.maxHp || 100}
                </span>
              </div>
              <div className="w-full h-6 border-2 border-black bg-gray-200 dark:bg-zinc-800 relative">
                 <motion.div 
                   initial={{ width: 0 }}
                   animate={{ width: `${((user?.hp || 50) / (user?.maxHp || 100)) * 100}%` }}
                   className="h-full bg-red-500 border-r-2 border-black" 
                 />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
             <Card variant="brutal" className={`p-0 overflow-hidden ${darkMode ? 'bg-zinc-900 border-white' : 'bg-white border-black'}`}>
               ...
               <div className="p-3 grid grid-cols-2 gap-2">
                 {['Tasks', 'Skills', 'Map', 'Shop', 'Social', 'Settings'].map((item) => (
                   <Button key={item} variant="brutal" size="sm" className="justify-start text-xs font-bold uppercase h-9 w-full shadow-[2px_2px_0px_0px_#000]">
                     {item}
                   </Button>
                 ))}
               </div>
          ...
          <div className="lg:col-span-6 space-y-6">
                 ...
                 {tasks.slice(0, 3).map((task) => (
                   <div key={task.id} className={`flex items-center justify-between p-3 border-2 border-black shadow-[4px_4px_0px_0px_#000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_#000] transition-all ${darkMode ? 'bg-zinc-800' : 'bg-white'}`}>
                     <div className="flex items-center gap-3">
                       <div className={`h-3 w-3 border border-black ${task.priority === 'high' ? 'bg-red-500' : 'bg-blue-500'}`} />
                       <div>
                     ...
                     <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-green-500 hover:text-white border border-transparent hover:border-black rounded-none">
            ...
            {/* NonNegotiables and Internship quest removed from dashboard */}
